...
public class Experiments {
    ...
    public static void mainExperiment(String directory) throws IOException {
        double[] EPSILON_VALUES = new double[]{1,0.5,0.1,0.05,0.01,0.005};
        int[] K_VALUES = new int[]{10,20,50,100,200};
        StringBuilder sb = new StringBuilder();
        for (File file : listFiles(directory)) for (int k : K_VALUES){
            Instance instance = new Instance("Hyperedge", file.getCanonicalPath(), k);
        ...
        Files.writeString(Paths.get("results.csv"), sb.toString());
    }
}
